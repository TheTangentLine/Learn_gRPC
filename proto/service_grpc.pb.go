...
const (
	Broadcast_CreateConnection_FullMethodName = "/proto.Broadcast/CreateConnection"
...
type BroadcastClient interface {
	CreateConnection(ctx context.Context, in *Connect, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Message], error)
...
func (c *broadcastClient) CreateConnection(ctx context.Context, in *Connect, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Message], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Broadcast_ServiceDesc.Streams[0], Broadcast_CreateConnection_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type Broadcast_CreateConnectionClient = grpc.ServerStreamingClient[Message]
...
type BroadcastServer interface {
	CreateConnection(*Connect, grpc.ServerStreamingServer[Message]) error
...
func (UnimplementedBroadcastServer) CreateConnection(*Connect, grpc.ServerStreamingServer[Message]) error {
...
func _Broadcast_CreateConnection_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(Connect)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(BroadcastServer).CreateConnection(m, &grpc.GenericServerStream[Connect, Message]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Broadcast_CreateConnectionServer = grpc.ServerStreamingServer[Message]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _Broadcast_CreateConnection_Handler,
			ServerStreams: true,
		},
	},
